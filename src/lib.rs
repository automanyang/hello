...
#[proc_macro_attribute]
pub fn invoke2_interface_with_persistency(_attr: TokenStream, input: TokenStream) -> TokenStream {
...
#[proc_macro_attribute]
pub fn invoke2_interface(_attr: TokenStream, input: TokenStream) -> TokenStream {
    // let derive_serde = parse_macro_input!(attr as DeriveSerde);
    let ItemTrait {
        attrs,
        vis,
    ...
    let request_ident = Ident::new(&format!("{}Request", trait_ident), trait_ident.span());
    let request_ident_vect: Vec<_> = idents_collected
        .iter()
        .map(|_| request_ident.clone())
        .collect();
    ...
    let output2 = if cfg!(feature = "adapter") {
            ...
            S: #trait_ident + 'static,
        {
            ...
            fn serve(&mut self, ctx: Option<servant::Context>, req: Vec<u8>) -> Vec<u8> {
                ...
                let reps = match req {
                    #(
                        #request_ident_vect::#fn_ident_camel_vec{ #(#args_vec)* } =>
    ...
    let output3 = if cfg!(feature = "terminal") {
        ...
        impl #proxy_ident {
            ...
            pub async fn #fn_ident_vec(
                ...
                let request =  #request_ident_vect::#fn_ident_camel_vec { #(#args_vec)* };
                let response = self
                    .2
                    .invoke(Some(self.0.clone()), Some(self.1.clone()), bincode::serialize(&request).unwrap())
                    .await;
...
#[proc_macro_attribute]
pub fn report2_interface(_attr: TokenStream, input: TokenStream) -> TokenStream {
    let ItemTrait {
        attrs,
        vis,
        unsafety,
...
#[cfg(feature = "query")]
#[proc_macro_attribute]
pub fn query2_interface(_attr: TokenStream, input: TokenStream) -> TokenStream {
    let ItemTrait {
        attrs,
        vis,
        unsafety,
...
#[proc_macro_attribute]
pub fn notify2_interface(_attr: TokenStream, input: TokenStream) -> TokenStream {
    let ItemTrait {
        attrs,
        vis,
        unsafety,
